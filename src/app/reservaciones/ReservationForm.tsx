...
export default function ReservationForm() {
  ...
  const validateStep = (fromStep: number): boolean => {
    ...
    if (fromStep === 3) {
      ...
      if (paymentMethod === "Zelle" && (!paymentData.bank || !paymentData.reference)) {
        setErrorMsg("Ingresa el nombre del titular y la referencia de Zelle");
        return false;
      }
      if (paymentMethod === "Pago Movil") {
        if (!paymentData.bank) {
          setErrorMsg("Selecciona tu banco de origen");
          return false;
        }
        if (!paymentData.paymentId || paymentData.paymentId.length < 5) {
          setErrorMsg("Ingresa tu número de cédula");
          return false;
        }
        const phoneDigits = (paymentData.paymentPhone || "").replace(/\D/g, "");
        if (phoneDigits.length < 11) {
          setErrorMsg("Ingresa tu número de teléfono completo (11 dígitos)");
          return false;
        }
        if (!paymentData.reference) {
          setErrorMsg("Ingresa el número de referencia del pago");
          return false;
        }
      }
      if (paymentMethod === "Binance" && (!paymentData.binanceUser || !paymentData.reference)) {
        setErrorMsg("Completa los datos de Binance");
        return false;
      }
      if (paymentMethod === "Efectivo" && (!paymentData.billDenomination)) {
        setErrorMsg("Indica la denominación de tus billetes");
        return false;
      }
      
      if (paymentMethod !== "Efectivo" && !proofFile) {
        setErrorMsg("Debes adjuntar el comprobante de pago");
        return false;
      }
  ...
  const handleBeforeStepChange = (fromStep: number, toStep: number) => {
    if (toStep < fromStep) {
      setErrorMsg("");
      return true;
    }
    return validateStep(fromStep);
  };
  ...
  const stepIndex = checkoutStep === "CONTACT" ? 1 : checkoutStep === "PLAN" ? 2 : 3;

  return (
    <>
      <div style={{ textAlign: "center", marginBottom: "36px" }}>
        <h1 className="heading-1" style={{ fontSize: "2.5rem" }}>Reserva tu Cita</h1>
        <p className="text-muted" style={{ fontSize: "1.05rem", maxWidth: "560px", margin: "8px auto 0 auto" }}>
      ...
      <Stepper
        ...
        stepTitles={["Datos", "Plan", "Pago"]}
        onStepChange={(newStep) => {
          if (newStep === 1) setCheckoutStep("CONTACT");
          else if (newStep === 2) setCheckoutStep("PLAN");
          else if (newStep === 3) setCheckoutStep("PAYMENT");
        }}
      ...
      >
        ...
        <Step>
          {errorMsg && (
            <div style={{ backgroundColor: "#fee2e2", color: "#b91c1c", padding: "12px 16px", borderRadius: "10px", marginBottom: "20px", fontSize: "0.92rem", border: "1px solid #fecaca" }}>
              {errorMsg}
            </div>
          )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            {plans.length === 0 ? <p>Cargando planes...</p> : (
              <div style={{ display: "grid", gap: "12px" }}>
                {plans.map(p => (
                  <label key={p.id} style={{ display: "flex", alignItems: "center", gap: "12px", padding: "16px", border: selectedPlanId === p.id ? "2px solid var(--color-primary, #b08b6e)" : "1px solid var(--color-border)", borderRadius: "12px", cursor: "pointer", backgroundColor: selectedPlanId === p.id ? "rgba(176, 139, 110, 0.06)" : "transparent", transition: "all 0.2s ease" }}>
            ...
            {addons.length > 0 && (
              <div style={{ marginTop: "16px", padding: "16px", backgroundColor: "var(--color-surface)", borderRadius: "12px", border: "1px solid var(--color-border)" }}>
        ...
        <Step>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            <div style={{ padding: "16px", backgroundColor: "var(--color-surface)", borderRadius: "12px", border: "1px solid var(--color-border)", marginBottom: "16px" }}>
            ...
            {paymentMethod === "Pago Movil" && (
              <div style={{ backgroundColor: "var(--color-bg-primary)", padding: "16px", borderRadius: "12px", border: "1px solid var(--color-border)" }}>
